...
비즈니스 영역에서 사용하는 controller, service 같은 뻔한, 패턴이 있는 작업들은 그 수도 많고 문제가 발생하는 곳이 명확하기 때문에 자동을 사용하는 것이 좋지만 데이터 베이스 연결, 공통 로그 처리 같은 기술 지원 영역에 대한 빈들은 그 수가 적을 뿐더러 광범위한 곳에 영향을 미치기 때문에 수동 빈을 사용해서 이들이 사용되는 곳을 명확히 하는 것이 좋다.
